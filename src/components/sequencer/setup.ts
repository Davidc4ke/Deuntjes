...
export function mountSequencer(root: HTMLElement, options: MountOptions = {}): () => void {
  ...
  function updateCursorVisuals() {
    ...
    if (_cachedCursorBtn) {
      _cachedCursorBtn.style.top = Math.max(0, Math.min(state.cursor * rowH - 11, h - 22)) + "px";
